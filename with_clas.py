...
class Drill(Questions):
    def __init__(self, path):
        ...
        self.check_answers_frame.grid_columnconfigure(0, weight=1, minsize=900)

        self.top_frame.grid_columnconfigure(2, weight=1, minsize=900)
        self.j = 0
        self.top_frame.grid(row=0, sticky='ew', columnspan=3)
        self.question_frame.grid(row=1, sticky='nsew')
        self.answers_frame.grid(row=2, sticky='nsew')
        self.check_answers_frame.grid(row=3, sticky='nsew')
        self.counts = 0
        self.path_list = {}
        self.var3 = IntVar()
        self.var3.set(0)
        # top_frame widgets
        self.open_file_button = Button(self.top_frame, text="Open File", command=self.open_file, bg="grey")
    ...
    def drill(self, counts):

        question_frame = Frame(self.root, width=900, height=115)
        answers_frame = Frame(self.root, width=900, height=220)
        ...
        for i in range(len(self.questions)):
            ...
            self.question_label = Label(question_frame, wraplength=800, text=self.questions[self.i], height=5, pady=10,
                                        padx=30)
            self.question_label.grid(row=0, sticky="NSW", column=0)
            self.question_a.grid(row=0, sticky="NSW", column=0)
            self.question_b.grid(row=1, sticky="NSW", column=0)
            self.question_c.grid(row=2, sticky="NSW", column=0)
            self.question_d.grid(row=3, sticky="NSW", column=0)
